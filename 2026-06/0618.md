...
@Service
@RequiredArgsConstructor    // final 필들르 파라미터로 받는 생성자를 자동생성(lombok)
public class MenuService {
    ...
    public MenuResponseDTO findMenuByMenuCode(int menuCode) {
        ...
        // foundMenu의 값을 MenuResponseDTO에 복사해서 새 DTO 객체를 만들어준다
...
- 이 메서드 안에서 일어나는 DB 작업을 하나의 작업(트랜잭션)으로 묶어주는 역할을 한다.
- 즉, 도중에 하나라도 실패하면 전부 원래대로 되돌려라(rollback)라는 의미
